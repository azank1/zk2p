...
// ============================================================================
// Phase 2B: New Account Structures (Side-by-Side Integration)
// ============================================================================
...
pub mod market {
    ...
    // ========================================================================
    // Phase 2B: New Instructions (Side-by-Side with Phase 2A)
    // ========================================================================

    /// Initialize the market account for Phase 2B
    pub fn initialize_market(ctx: Context<InitializeMarket>) -> Result<()> {
        ...
        msg!("Market: Initialized Phase 2B market for mint: {}", market.token_mint);
        msg!("Market: Authority set to: {}", market.authority);
        Ok(())
    }

    /// Initialize Phase 2B order book with CritBit tree
    pub fn initialize_order_book_v2(ctx: Context<InitializeOrderBookV2>) -> Result<()> {
        let order_book = &mut ctx.accounts.order_book_v2;
        let market = ctx.accounts.market.key();
        let token_mint = ctx.accounts.token_mint.key();
        
        // Initialize OrderBookV2 with CritBit trees
        // Use double deref to assign to Account wrapper
        **order_book = OrderBookV2::new(market, token_mint, token_mint);
        
        msg!("Market: Initialized OrderBookV2 for mint: {}", token_mint);
        msg!("Market: Supports {} price levels", OrderBookV2::MAX_PRICE_LEVELS);
        msg!("Market: CritBit trees initialized for bids and asks");
        Ok(())
    }

    /// Place a limit order using Phase 2B order book
...
// ============================================================================
// Phase 2B: Account Validation Structures
// ============================================================================
...
#[derive(Accounts)]
pub struct InitializeOrderBookV2<'info> {
    #[account(
        init,
        payer = payer,
        space = 8 + std::mem::size_of::<OrderBookV2>(),
